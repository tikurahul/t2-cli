      ...
      return new Promise((resolve, reject) => {
        ...
        this.connection.exec(commands.sysupgrade(updatePath), (err, remoteProc) => {
          if (err) {
            ...
            logs.info('Applying OpenWRT Update...')
            /*
            I found that resolving immediately causes the `sysupgrade` command to
            abort. Experimentally, it looks like 120 seconds is about the right amount
            of time for the sysupgrade to advance to a point where it does not mind
            that the parent process that exec'ed the command has been killed.
            */
