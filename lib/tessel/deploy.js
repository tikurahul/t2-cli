      ...
      .then(function(remoteProcess) {
        actions.findProject(opts).then(function(project) {
          ...
          actions.tarBundle(opts).then(function(bundle) {
            ...
            var bar = new ProgressBar('  deploying [:bar] :percent :etas remaining', {
              complete: '=',
              incomplete: ' ',
              width: 20,
              total: bundle.length/1000
            });

            var prgTimeout = setInterval(function() {
              actions.getTransferState(tessel, filepath)
              .then(function(state) {
                bar.update(state/bar.total);
                if (state === bar.total) {
                  clearInterval(prgTimeout);
                  deploymentComplete();
                }
              });
            }, 1000);
...
actions.getTransferState = function(tessel, filepath) {
  return tessel.simpleExec(commands.getFolderSize(filepath))
  .then(function(sizeString) {
    var size = Number(sizeString.trim().split('\t')[0]);
